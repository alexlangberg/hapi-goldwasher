    ...
    handler: function(request, reply) {
      getResponse(server, options, request, function(error, result) {
        ...
        if (request.query.output === 'xml') {
          return reply(result).type('text/xml');
        }

        if (request.query.output === 'atom') {
          return reply(result).type('text/xml').type('application/atom+xml');
        }

        if (request.query.output === 'rss') {
          return reply(result).type('text/xml').type('application/rss+xml');
        }
